...
function linkObjectsToHtml(linkObjects) {
	...
	.map(function (linkObject) {
		...
		<li data-search="${linkObject.titleAndAuthor.toLowerCase()}">
			...
			<a href="${linkObject.feedUrl}" onclick="return copy(this, '${linkObject.feedUrl}')" class="icon-link" title="Copy RSS URL">
				<img src="./rss.svg" alt="copy rss feed url for ${linkObject.titleAndAuthor}" class="icon rss-icon">
			</a>
			<a href="http://cloud.feedly.com/#subscription%2Ffeed%2F${encodeURIComponent(linkObject.feedUrl)}" target="_blank" class="icon-link" title="Open in feedly">
				<img src="${feedlyIconUrl}" alt="follow ${linkObject.titleAndAuthor} in feedly" class="icon feedly-icon">
			</a>
		</li>`
	})
	.sort()
